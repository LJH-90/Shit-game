...
- 밸런스 로그: `World.drain_log()` → 스테이지 종료(클리어/게임 오버)마다 1 dict. molgam 이 `balance_log.jsonl`(세이브 파일 옆)에 한 줄씩 append.
  v2.1 열: `player.upgrades` {str,agi,int,rate,shield,life} · `player.legacy` {hat,gloves,suit,shoes} · `score_spent`(`_shop_select`, 마을/상점 동안은 `_bal_carry`) · `deaths_pit` · `coins_lost`(TTL 만료 코인 금액) · `mult`(stage["mult"] 반올림 3자리) · molgam 이 `data` {stages, config} = 각 파일의 `config_version` 추가. `_bal_enemy` 는 `e.boss and e.mid` 를 `"mid"` 열로 분리(`_make_enemy(..., mid=True)` 가 로그 전에 `e.mid` 를 세운다).
- 데이터 리셋(v2.1): `molgam.load_config_file` 은 동봉 파일의 `config_version` 이 더 크면 `updater.reset_blocks(data, bundled, DATA_RESET_BLOCKS[name])` — dict 재귀·숫자/불/문자열 덮어쓰기(`KEEP_TEXT_KEYS` name/title/boss_title/boss_titles/label/hotkey 의 문자열·문자열 리스트는 유지), dict 리스트는 인덱스별 병합(동봉본이 길면 추가, 사용자 것이 길면 유지), 그 외 리스트/스칼라는 교체, 파일에만 있는 키는 유지. stages.json 도 `config_version`(2) 을 갖는다. `DATA_RESET_BLOCKS` = config.json ("equipment", "characters") (config_version 3) / stages.json 데이터 블록 전부.
- 무기 상점 재고는 마을 방문(스테이지)마다 새로 뽑고, 그 마을 안에서는 유지된다(`shop_stock_stage`).

#### v2.0 구현 메모 (밸런스·경제·시각 — 계약 변경분)
- 난이도 곡선: `DIFF_CURVE` (game.py, stages.json `"curve"` 로 덮어씀) 는 easy / normal 에만 존재. hp/speed/count/proj/money 를 직접 지정하고 1~3스테이지에 `warmup`(hp·speed) / `count_warmup`(count) 을 곱한다(무한 사이클 보너스 +0.25/+0.05/+0.1 은 그대로). `hp_per_stage`(easy .22 / normal .28) · `boss_hp_per_stage` · `speed_cap`(2.2 / 2.6) 도 곡선에서 가져와 `stage["hp_per_stage"|"boss_hp_per_stage"|"speed_cap"]` 로 빌드 시점에 확정한다. v2.1 `boss_keep`(easy 1.0 / normal 0.5, hard+ 0) → `stage["boss_keep"]`: `_after_death` 가 살아 있는 보스의 잃은 HP 를 `_boss_carry` {rank, mid, lost} 로 남기고, `_spawn_boss`/`_spawn_mid_boss` 뒤 `_apply_boss_carry` 가 같은 rank·mid 의 새 보스에 `hp = hp_max - round(lost x boss_keep)` 을 적용(스테이지 시작에 초기화). hard / harder / hell / crazy 는 항목이 없으므로 `DIFF_MULT` 공식·progression 0.35·ENEMY_SPEED_STAGE_CAP 3.0 그대로(숫자 불변, selftest 로 고정). stages.json `"difficulty"` 표는 legacy(미사용).
